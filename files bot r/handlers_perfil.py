...
from ia_coach import pedir_plan_semanal
from analisis_runner import recomendar_metodologia
from vdot import obtener_metodologia_vdot
from handlers_entrenamiento import PLANES, formatear_plan_semanal
from validaciones import (
    validar_edad, validar_kilometraje, validar_minutos_sesion,
    validar_ritmo_facil, validar_marca,
)

# Mapea la columna real de usuarios a la distancia que usa validar_marca,
# para el flujo de "actualizar un dato" (donde solo se conoce el nombre
# de columna, no la distancia como en la encuesta).
COLUMNA_A_DISTANCIA_MARCA = {
    "marca_5k": "5K",
    "marca_10k": "10K",
    "marca_media_maraton": "Media maratón",
}
...
async def recibir_edad(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ok, edad, error = validar_edad(update.message.text)
    if not ok:
...
async def recibir_kilometraje(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if es_saltar(texto):
        ...
        ok, km, error = validar_kilometraje(texto)
        if not ok:
            await update.message.reply_text(error)
            return KILOMETRAJE
        context.user_data["kilometraje"] = km
    await update.message.reply_text(
...
async def recibir_ritmo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    texto = update.message.text
    if es_saltar(texto):
        context.user_data["ritmo_facil"] = None
    else:
        ok, ritmo, error = validar_ritmo_facil(texto)
        if not ok:
            await update.message.reply_text(error)
            return RITMO
        context.user_data["ritmo_facil"] = ritmo
    teclado = ReplyKeyboardMarkup(
...
async def recibir_marca_5k(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if es_saltar(texto):
        ...
        ok, marca, error = validar_marca(texto, "5K")
        if not ok:
            await update.message.reply_text(error)
            return MARCA_5K
        context.user_data["marca_5k"] = marca
...
async def recibir_marca_10k(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if es_saltar(texto):
        ...
        ok, marca, error = validar_marca(texto, "10K")
        if not ok:
            await update.message.reply_text(error)
            return MARCA_10K
        context.user_data["marca_10k"] = marca
...
async def recibir_marca_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if es_saltar(texto):
        ...
        ok, marca, error = validar_marca(texto, "Media maratón")
        if not ok:
            await update.message.reply_text(error)
            return MARCA_MEDIA
        context.user_data["marca_media"] = marca
    return await preguntar_objetivo(update, context)
...
async def recibir_minutos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ok, minutos, error = validar_minutos_sesion(update.message.text)
    if not ok:
        await update.message.reply_text(error)
        return MINUTOS
...
async def recibir_explicacion_metodologia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if texto in ("sí", "si", "sí.", "si.", "sí!", "si!"):
        ...
        resultado_plan = pedir_plan_semanal(contexto_json)

        if resultado_plan:
            guardar_plan_texto(telegram_id, resultado_plan["resumen"])
            guardar_plan_sesiones(telegram_id, resultado_plan["sesiones"])
            texto_plan = formatear_plan_semanal(resultado_plan["resumen"], resultado_plan["sesiones"])
            await enviar_mensaje_largo(update, texto_plan, parse_mode=ParseMode.MARKDOWN)
        else:
            await update.message.reply_text(
...
async def recibir_valor_dato(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if columna == "dias_entrenamiento":
        ...
        ok, valor, error = validar_kilometraje(texto)
        if not ok:
            await update.message.reply_text(error)
            return DATO_VALOR

    elif columna == "minutos_por_sesion":
        ok, valor, error = validar_minutos_sesion(texto)
        if not ok:
            await update.message.reply_text(error)
            return DATO_VALOR

    elif columna == "ritmo_facil":
        ok, valor, error = validar_ritmo_facil(texto)
        if not ok:
            await update.message.reply_text(error)
            return DATO_VALOR

    elif columna in COLUMNA_A_DISTANCIA_MARCA:
        ok, valor, error = validar_marca(texto, COLUMNA_A_DISTANCIA_MARCA[columna])
        if not ok:
            await update.message.reply_text(error)
            return DATO_VALOR
