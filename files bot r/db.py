...
# 0=Lunes ... 6=Domingo. Se usa para poder mostrar las sesiones siempre
# en el mismo orden sin importar en qué orden las haya devuelto la IA.
DIAS_ORDEN = {
...
def guardar_plan_sesiones(telegram_id, sesiones):
    """
    Reemplaza por completo las sesiones de plan guardadas para este
    corredor (borra las viejas, inserta las nuevas). Se llama junto con
    guardar_plan_texto (que guarda el resumen y marca
    plan_generado_en) cada vez que se regenera el plan semanal.
    """
    conexion = sqlite3.connect(DB_PATH)
    cursor = conexion.cursor()
    cursor.execute("DELETE FROM plan_sesiones WHERE telegram_id = ?", (telegram_id,))
    for sesion in sesiones:
        orden = DIAS_ORDEN.get((sesion.get("dia") or "").strip().lower(), 99)
        cursor.execute(
            "INSERT INTO plan_sesiones "
            "(telegram_id, dia, orden, nombre, tipo, descripcion, duracion_min, distancia_km, ritmo_objetivo) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                telegram_id, sesion.get("dia"), orden, sesion.get("nombre"),
                sesion.get("tipo"), sesion.get("descripcion"),
                sesion.get("duracion_min"), sesion.get("distancia_km"),
                sesion.get("ritmo_objetivo"),
            ),
        )
    conexion.commit()
    conexion.close()
...
def obtener_plan_sesiones(telegram_id):
    """
    Devuelve las sesiones del plan vigente, ordenadas de Lunes a
    Domingo, como lista de dicts. Lista vacía si no hay plan guardado.
    """
    conexion = sqlite3.connect(DB_PATH)
    conexion.row_factory = sqlite3.Row
    cursor = conexion.cursor()
    cursor.execute(
        "SELECT dia, nombre, tipo, descripcion, duracion_min, distancia_km, ritmo_objetivo "
        "FROM plan_sesiones WHERE telegram_id = ? ORDER BY orden",
        (telegram_id,),
    )
